 ...
 * CLI version: 2.12.0
 *
 * Generated: Wed, 30 Aug 2023 13:17:21 GMT
 */
...
var APP_com_domain_app_player3 = (function () {
  ...
  class App extends Lightning$1.Component {
    ...
    _init() {
      // Listen for the keydown event at the document level
      this._onKeyDown = this._handleKeyDown.bind(this);
      document.addEventListener("keydown", this._onKeyDown);
    }
    _handleKeyDown(event) {
      if (event.keyCode === 13) {
        // Enter key was pressed, check if the event target is the "Play" button
        const playButton = this.tag("PlayButton");
        if (event.target === playButton.getRef()) {
          // Play the video
          this.playVideo();
        }
      }
    ...
    _firstActive() {
      // Focus on the "Play" button when the component becomes active
      this.tag("PlayButton").setSmooth("alpha", 1);
    }
    playVideo() {
      const url = "http://devimages.apple.com/iphone/samples/bipbop/bipbopall.m3u8";

      // Set the source URL directly for the VideoItem component
    ...
    _inactive() {
      // Remove the keydown event listener when the component becomes inactive
